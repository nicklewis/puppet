  ...
  module Util
    class CommandLine
      ...
      def self.available_subcommands
        Puppet::Application.available_applications
      end
      ...
      def execute
        if available_subcommands.include? subcommand_name
          app_class = Puppet::Application.find(subcommand_name)
          app = app_class.new(self)
          ...
          if $delayed_deprecation_warning_for_p_u_cl.is_a? String
            Puppet.deprecation_warning($delayed_deprecation_warning_for_p_u_cl)
            $delayed_deprecation_warning_for_p_u_cl = true
          end

          app.run
        elsif path_to_subcommand = external_subcommand(subcommand_name)
          system(path_to_subcommand, *args)
        else
          puts "Error: Unknown Puppet subcommand '#{subcommand_name}'" if subcommand_name
          puts "See 'puppet help' for help on available puppet subcommands"
        end
      end

      def external_subcommand(subcommand_name)
        external_command = "puppet-#{subcommand_name}"

        require 'puppet/util'
        Puppet::Util.which(external_command)
      end

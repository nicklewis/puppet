...
describe Puppet::Interface do
  ...
  describe "with face-level options" do
    ...
    it "should work when two actions have the same option" do
      face = subject.new(:with_options, '0.0.1') do
        action :foo do when_invoked {|options| true } ; option "--quux" end
        action :bar do when_invoked {|options| true } ; option "--quux" end
      end
  ...
  describe "with inherited options" do
    let :parent do
      ...
      parent.action(:parent_action) do when_invoked {|options| true } end
      parent
    end
    ...
    let :face do
      ...
      face.option("--local")
      face.action(:face_action) do when_invoked {|options| true } end
      face
    end
